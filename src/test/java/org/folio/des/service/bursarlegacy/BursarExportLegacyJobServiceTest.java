...
class BursarExportLegacyJobServiceTest {
  ...
  static {
    ...
    nonBursar1.setExportTypeSpecificParameters(
    ...
    nonBursar2.setExportTypeSpecificParameters(
      new ExportTypeSpecificParametersWithLegacyBursar().bursarFeeFines(null)
    );
    ...
    isBursar.setExportTypeSpecificParameters(
      new ExportTypeSpecificParametersWithLegacyBursar()
        .bursarFeeFines(new LegacyBursarFeeFines().daysOutstanding(1))
    );
  }

  @Test
  void testGetBlankQuery() {
    List<JobWithLegacyBursarParameters> items = List.of(
      nonBursar1,
      nonBursar2,
      nonBursar3,
      isBursar
    ...
    assertEquals(isBursar.getId(), legacyJobCollection.get(0).getId());
  }

  private static List<Arguments> hasLegacyParametersArguments() {
    return List.of(
      Arguments.of(isBursar, true),
      Arguments.of(nonBursar1, false),
      Arguments.of(nonBursar2, false),
  ...
  void testHasLegacyBursarParameters(
    JobWithLegacyBursarParameters job,
    boolean expected
  ) {
    assertEquals(
      expected,
      BursarExportLegacyJobService.hasLegacyBursarParameters(job)
    );
  }
}
